...
def parse_plan_markdown(text):
    ...
    for raw_line in text.splitlines():
        ...
        if task_match:
            ...
            # Optional "Title | resource URL | description" shorthand —
            # each part after the title is optional.
            parts = [p.strip() for p in raw.split('|')]
            task_title = parts[0]
            task_url = parts[1] if len(parts) > 1 and parts[1] else None
            task_description = parts[2] if len(parts) > 2 and parts[2] else None
            if task_title:
                ...
                for topic in subj["topics"]:
                    ...
                    for task in topic["tasks"]:
                        connection.execute(
                            ...
                            VALUES (?, ?, 30, 2, 'Medium', ?, ?, ?)
                            """,
                            (task["title"], task["description"], subj["deadline"],
